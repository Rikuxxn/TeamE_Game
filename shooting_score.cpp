...
void SetSTPass(int nSTAPass) 
{
	...
	for (int nCntSTPass = 0; nCntSTPass < MAX_NUM_SCORE; nCntSTPass++)
	{
		...
		pVtx[1].tex = D3DXVECTOR2(0.1f + (0.1f * aPosTexU[nCntSTPass]), 0.0f);
		pVtx[2].tex = D3DXVECTOR2(0.0f + (0.1f * aPosTexU[nCntSTPass]), 1.0f);
		pVtx[3].tex = D3DXVECTOR2(0.1f + (0.1f * aPosTexU[nCntSTPass]), 1.0f);
		pVtx += 4;
	}
	// 頂点バッファをアンロックする
	g_pVtxBuffSTPass->Unlock();
