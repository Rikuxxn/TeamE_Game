...
//グローバル
PasswordParticle g_aParticle[MAX_PARTICLE];//パーティクルの情報
		...
		if (g_aParticle[nCntParticle].bUse == true)
		{
			...
			for (nCntAppear = 0; nCntAppear < 20; nCntAppear++)
			{
				if (g_aParticle[nCntParticle].nType == 0)//爆発
				{
					...
					//エフェクトの設定
					SetPasswordEffect(pos, move, col, fRadius, nLife,0);
				}
				...
				else if (g_aParticle[nCntParticle].nType == 13)//パーティクルの爆発
				{
					//位置の設定
					pos = g_aParticle[nCntParticle].pos;
			...
			for (nCntAppear = 0; nCntAppear < 4; nCntAppear++)
			{
				if (g_aParticle[nCntParticle].nType == 2)//火炎放射的なやつ
				{
					...
					//エフェクトの設定
					SetPasswordEffect(pos, move, col, fRadius, nLife,1);
				}
			}
...
void SetPasswordParticle(D3DXVECTOR3 pos,int nType)
{
	int nCntParticle;
	VERTEX_2D* pVtx = 0;
	LPDIRECT3DDEVICE9 pDevice;
