...
void SetBallPass(int nBallAPass) 
{
	...
	for (int nCntBallPass = 0; nCntBallPass < MAX_NUM_SCORE; nCntBallPass++)
	{
		...
		pVtx[1].tex = D3DXVECTOR2(0.1f + (0.1f * aPosTexU[nCntBallPass]), 0.0f);
		pVtx[2].tex = D3DXVECTOR2(0.0f + (0.1f * aPosTexU[nCntBallPass]), 1.0f);
		pVtx[3].tex = D3DXVECTOR2(0.1f + (0.1f * aPosTexU[nCntBallPass]), 1.0f);
		pVtx += 4;
	}
	// 頂点バッファをアンロックする
	g_pVtxBuffBallPass->Unlock();
