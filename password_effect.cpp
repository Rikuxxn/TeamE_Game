...
//グローバル
LPDIRECT3DTEXTURE9 g_pTexturePasswordEffect = NULL;     //テクスチャへのポインタ
LPDIRECT3DVERTEXBUFFER9 g_pVtxBuffPasswordEffect = NULL;//頂点バッファへのポインタ
PasswordEffect g_aEffect[MAX_EFFECT];					//エフェクトの情報
...
void InitPasswordEffect(void)
{
	...
	g_pVtxBuffPasswordEffect->Lock(0, 0, (void**)&pVtx, 0);
	...
	//頂点バッファをアンロック
	g_pVtxBuffPasswordEffect->Unlock();
}
//エフェクトの終了
void UninitPasswordEffect(void)
{
	//テクスチャの破棄
	if (g_pTexturePasswordEffect != NULL)
	{
		g_pTexturePasswordEffect->Release();
		g_pTexturePasswordEffect = NULL;
	}

	//頂点バッファの破棄
	if (g_pVtxBuffPasswordEffect != NULL)
	{
		g_pVtxBuffPasswordEffect->Release();
		g_pVtxBuffPasswordEffect = NULL;
	}
}
//エフェクトの更新
...
void DrawPasswordEffect(void)
{
	...
	//頂点バッファをデータストリーム
	pDevice->SetStreamSource(0, g_pVtxBuffPasswordEffect, 0, sizeof(VERTEX_2D));

	//頂点フォーマットの設定
	pDevice->SetFVF(FVF_VERTEX_2D);

	//テクスチャの設定
	pDevice->SetTexture(0, g_pTexturePasswordEffect);
...
//エフェクトの設定処理
void SetPasswordEffect(D3DXVECTOR3 pos,D3DXVECTOR3 move, D3DXCOLOR col, float fRadius, int nLife,int nType)
{
	int nCntEffect;
	VERTEX_2D* pVtx=0;
